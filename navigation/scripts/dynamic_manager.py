...
class Manager():
	...
	def __move_to_next_point(self):
		...
		goal_for_controller.goal.x, goal_for_controller.goal.y = next_point.x, next_point.y
	...
	def __odom_update(self, data): 
		...
		if len(self.path) > 1:
			self.path = collections.deque(utils.transform(LineString(self.path), self.position, self.orientation).coords)
