...
class RRT():
    ...
    def Planning(self, start, goal, animation=True):
        ...
        self.nodeList = [start]
        dx = start.x - self.end.x
        dy = start.y - self.end.y
        d = math.sqrt(dx * dx + dy * dy)
        if  not self.st_linecheck(start,d):
            while True:
                # Random Sampling
...
rrt = RRT(start=[0, 0], goal=[10, 5],
              randArea=[-5, 5], obstacleList=[], obstacleList2=[])
do_RRT(rrt=rrt, obstacleList2=[], show_animation=True, start=[0, 0], goal=[10, 5])
